...
const operationId = {
  ...
  publishRecipe: (code: string, version: number): string => `publish_recipe:${code}:${version}`,
...
const output = (operation: string, field: "id" | "revision"): MutationOperationOutputReference => ({
...
function recipeTagHandle(manifest: ResolvedCatalogManifestV1, code: string): string | null {
  const matches = manifest.references.recipeTags.filter((reference) => reference.code === code)
  return matches.length === 1 ? `reference:recipe_tag:${matches[0]?.kind}:${code}` : null
}
...
function buildOperations(
  ...
  for (const price of priceBook.prices) {
    const dependency = foodFactPublishes.get(`${price.foodCode}:${price.foodFactVersionNumber}`)
  ...
  operations.push({
    ...
    input: {
      ...
      prices: canonicalPrices.map((price) => ({
        ...
        foodFactVersionId: bind(foodFactVersionHandle(price.foodCode, price.foodFactVersionNumber)),
  ...
  for (const mealOption of [...pack.mealOptions].sort((left, right) =>
    ...
    const tagHandles = uniqueCodes([
      ...
      ...mealOption.version.dishRoleCodes.map((code) => `reference:recipe_tag:dish_role:${code}`)
    ])
...
export function planCatalogMutations(
  ...
  const orderedOperations = orderMutationOperations(buildOperations(pack, manifest))
