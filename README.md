...
Rate limiting for the assistant and the planner endpoints uses `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN`, server-only like the Gemini values. Both windows are consumed inside one Redis Lua script, so the quota holds across serverless instances; production Gemini stays disabled until that shared limiter is configured. Without the pair the planner behaves exactly as before and the per-instance limiter remains available outside production. Never define `VITE_UPSTASH_*`.
