...
-- A v4 revision must persist. This is the guard that would have broken plan generation outright if
-- the engine bump had shipped ahead of this migration: `persist_meal_plan_revision` rejects an
-- engine version it does not know, and rejection here is not a degraded plan, it is no plan.
select lives_ok(
  $$
  ...
  $$,
  'a planner-engine-v4 revision persists'
);
...
select lives_ok(
  $$
    select public.persist_meal_plan_revision(
      '91000000-0000-0000-0000-000000000001',
      '92000000-0000-0000-0000-000000000001', date '2026-09-07', 0, null,
      '95000000-0000-0000-0000-000000000002',
      pg_temp.revision_payload(
        'generation', null, repeat('d', 64), 700000,
        '96000000-0000-0000-0000-000000000007',
        '96000000-0000-0000-0000-000000000006', date '2026-08-01', true,
        'planner-engine-v4'
      ),
      pg_temp.plan_items(true)
    )
  $$,
  'a second week persists so there is a history to read'
);

select is(
  jsonb_array_length(
    public.get_recent_meal_option_ids(
      '92000000-0000-0000-0000-000000000001', date '2026-09-14', 2
    )
  ),
  14,
  'a two-week lookback sees both weeks, fourteen distinct meals'
);

select is(
  jsonb_array_length(
    public.get_recent_meal_option_ids(
      '92000000-0000-0000-0000-000000000001', date '2026-09-14', 1
    )
  ),
  7,
  'a one-week lookback stops at the week before, not the one before that'
);

select is(
  public.get_recent_meal_option_ids(
    '92000000-0000-0000-0000-000000000001', date '2026-08-31', 2
  ),
  '[]'::jsonb,
  'a week is not its own history: only strictly earlier weeks count'
);

select is(
  public.get_recent_meal_option_ids(
    '92000000-0000-0000-0000-000000000001', date '2026-09-14', 0
  ),
  '[]'::jsonb,
  'a zero lookback asks for nothing rather than for everything'
);

select is(
  public.get_recent_meal_option_ids(gen_random_uuid(), date '2026-09-14', 2),
  '[]'::jsonb,
  'another household has no history here, and gets an empty array rather than null'
);

select ok(
  public.get_recent_meal_option_ids(
    '92000000-0000-0000-0000-000000000001', date '2026-09-14', 1
  ) @> to_jsonb('93000000-0000-0000-0000-000000000008'::uuid),
  'the identities returned are the meals the second week actually contains'
);

-- Contract of the read path itself, which is what keeps it inside RLS.
select is(
  (select prosecdef from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = 'get_recent_meal_option_ids'),
  false,
  'plan history is read as the caller, so RLS decides what it can see'
);
select is(
  (select proconfig from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = 'get_recent_meal_option_ids'),
  array['search_path=""'],
  'plan history pins an empty search_path like every other exposed function'
);
select ok(
  not has_function_privilege(
    'anon', 'public.get_recent_meal_option_ids(uuid,date,integer)', 'execute'
  ),
  'anonymous callers cannot read any household plan history'
);
select ok(
  has_function_privilege(
    'authenticated', 'public.get_recent_meal_option_ids(uuid,date,integer)', 'execute'
  ),
  'signed-in callers can, subject to RLS'
);
