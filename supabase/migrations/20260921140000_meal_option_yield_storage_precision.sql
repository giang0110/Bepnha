-- meal_option_recipes.quantity_multiplier is NUMERIC(18,6), so repeating ratios are rounded on write.
-- Allow only the small yield drift implied by that storage precision while rejecting material mismatch.
